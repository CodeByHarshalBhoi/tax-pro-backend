...
const GetFederalTaxRatesByYearAndStatus = async (req,res) =>{
    try {
        const federalTaxBracket = await FederalTaxRates.find({year:req.body.year,filling_status:req.body.filling_status});
...
const GetAllFederalTaxRatesByYearAndStatus = async (req,res) =>{
  try {
      const federalTaxBracket = await FederalTaxRates.find({year:req.query.ye ar});
