...
const AddProfileBasic = async (req, res) => {
  console.log("AddProfileBasic",req.auth.Id)
    try {
      let profile = new ProfileBasic({ ...req.body,parent_id:req.auth.Id });
      await profile.save(); 
...
console.log()                let socialSecurityIncome = new SocialSecurityIncome({
                    person_id: profile.person_id,
                    client_info_id: profile._id,
...
const GetClientProfile = async (req, res) => {
    try {
      ...
      const Profile = await ProfileBasic.findById(req.body.id).populate('person_id');
...
const getAllProfiles = async (req, res) => {
    ...
    //   });
      ...
      return res.status(200).json({
        message: 'ProfileBasics fetched successfully',
        info,
      });
    } catch (error) {
        console.log("error",error)
